<template>
  <div>
    ...
    <p class="page-subtitle">Kelola data diri dan keamanan akun Anda</p>
    ...
    <div v-else class="grid-3">
      ...
      <div class="card flex flex-col items-center text-center">
        <div class="profile-avatar mb-4">
          <span>{{ form.nama_siswa?.charAt(0).toUpperCase() }}</span>
        </div>
        <h2 class="font-bold" style="font-size:16px">{{ form.nama_siswa }}</h2>
        <p class="text-muted text-small mb-3">NIS: {{ form.nis }}</p>
        <span class="badge badge-primary">Siswa Aktif</span>
      </div>
        ...
        <form @submit.prevent="updateProfile" class="mb-6">
          <div class="form-grid">
            <div class="form-group">
              ...
              <input v-model="form.nama_siswa" type="text" class="form-control" disabled />
            </div>
            <div class="form-group">
              <label class="form-label">Kelas</label>
              <input :value="form.kelas?.nama_kelas" type="text" class="form-control" disabled />
            </div>
            <div class="form-group">
              <label class="form-label">Jenis Kelamin</label>
              <input :value="form.jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan'" type="text" class="form-control" disabled />
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <input v-model="form.tempat_lahir" type="text" class="form-control" disabled />
            </div>
            <div class="form-group">
              <label class="form-label">Tanggal Lahir</label>
              <input v-model="form.tanggal_lahir" type="date" class="form-control" disabled />
            </div>
...
const form = ref({ nama_siswa: '', nis: '', kelas: {}, jenis_kelamin: '', nomor_hp: '', tempat_lahir: '', tanggal_lahir: '', alamat: '' })
...
async function loadData() {
  ...
  const { data } = await supabase.from('siswa').select('*, kelas(nama_kelas)').eq('id_siswa', idSiswa).single()
...
async function updateProfile() {
  ...
  if (!error) {
    ...
    auth.updateProfile({ nama: form.value.nama_siswa })
  } else {
